...
cat > test/test-page.tmp.html << EOL
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
&lt;/script&gt;
  ...
  <div class="footer">
    ...
    <p>Server running at: <a href="http://localhost:$PORT" target="_blank">http://localhost:$PORT</a></p>
  </div>
</body>
</html>
EOL

# Step 4: Use Vite's preview server (more reliable than other options)
echo "🚀 Starting server on port $PORT..."
cd dist && npx vite preview --port $PORT --host localhost &
SERVER_PID=$!
...
if [[ "$OSTYPE" == "darwin"* ]]; then
  # macOS
  open test/test-page.tmp.html
elif [[ "$OSTYPE" == "linux-gnu"* ]]; then
  # Linux
  xdg-open test/test-page.tmp.html
elif [[ "$OSTYPE" == "msys" || "$OSTYPE" == "win32" ]]; then
  # Windows
  start test/test-page.tmp.html
else
  echo "⚠️ Could not automatically open browser. Please open test/test-page.tmp.html manually."
fi

echo ""
echo "✅ Done! Magic Bar should be loaded in your browser."
echo "📌 If you see a 404 error, check that:"
echo "   1. The bootloader.min.js file exists in the dist directory"
echo "   2. The vite preview server is running correctly"
echo ""
